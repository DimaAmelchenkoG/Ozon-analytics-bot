...
## TODO / Чеклист проекта

- ✅ Поднять связку Telegram-бот + FastAPI backend.
- ✅ Подключить Ozon Seller API и получать аналитику продаж.
- ✅ Формировать контекст для LLM и сохранять снапшоты отчётов в `var/`.
- ✅ Сохранять выгрузки продаж в SQLite (`ozon_analytics_reports`, `ozon_sales`).
- ✅ Настроить тестовый сервер для автоматической выгрузки при запуске.
- ✅ Получать остатки по складам через `AnalyticsAPI_AnalyticsStocks`.
- ✅ Автоматически конвертировать выгрузку остатков в Excel для просмотра.


- ⬜ Добавить отдельную таблицу БД для остатков (`ozon_analytics_stocks`) и перепсывать туда при каждой выгрузке.
- ⬜ Добавить работу с нейро по остаткам
- ⬜ Интегрировать работу с остатками в нормальный сервер (загрузка отчета с озон + работа с бд + нейро)
